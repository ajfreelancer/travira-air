...
function ListElement({ title }) {
  return (
    <li className="text-base text-darkblue font-bold mb-3">{title}</li>
  );
}
          ...
          <div className="grid th:grid-cols-2 gap-10 th:gap-20 sm:grid-cols-1 ">
            <div className="content text-center th:text-left order-last th:order-1 flex flex-col items-center th:items-start">
              ...
              <p className="text-[1rem] text-grayonwhite mb-5 mt-1 leading-[1.8rem] w-[90%] lg:w-[28rem]">
                It is a matter of pride for Travira Air that we have one of the
                best air charter safety records in the region
              </p>
            ...
            <div className="bg-[url('./assets/imgs/service-image.jpg')] rounded-3xl h-[400px] w-[95%] mx-auto lg:h-auto bg-cover"/>
          </div>
        </div>
      </section>
    </div>
